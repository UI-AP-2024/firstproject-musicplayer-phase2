...
public class ArtistsListController implements Initializable , GeneralOperations {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ArrayList<UserAccount> artists;
        artists = ListenerController.getListenerController().getAllArtists();
        int j=0,i=0;
        for (UserAccount artist : artists) {
            Button button = new Button();
            button.setText(artist.getUserName());
            button.setStyle("-fx-background-color: #012c2c");
            button.setTextFill(Paint.valueOf("white"));
            gridPain.add(button,j,i);
            gridPain.getChildren().getLast().setOnMouseClicked(e->{
                FXMLLoader fxmlLoader1 = new FXMLLoader(HelloApplication.class.getResource("artistInfo-view.fxml"));
                Detail.selectArtist = (Artist) artist;
                try {
                    Detail.lastScene.push(HelloApplication.getStage().getScene());
