...
public class PlayListsController implements Initializable , GeneralOperations {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ArrayList<Playlist> playLists = ListenerController.getListenerController().showAllPlayList();
        int j=0,i=0;
        for(Playlist playlist : playLists){
            Button button = new Button();
            button.setText(playlist.getNameOfPlaylist());
            button.setStyle("-fx-background-color: #012c2c");
            button.setTextFill(Paint.valueOf("white"));
            gridPain.add(button,j,i);
            gridPain.getChildren().getLast().setOnMouseClicked(e->{
                Detail.selectPlayList = playlist.getNameOfPlaylist();
                FXMLLoader fxmlLoader = new FXMLLoader(HelloApplication.class.getResource("audioPlayList-view.fxml"));
                try {
                    Detail.lastScene.push(HelloApplication.getStage().getScene());
                    HelloApplication.getStage().setScene(new Scene(fxmlLoader.load()));
